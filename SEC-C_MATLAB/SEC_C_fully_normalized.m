...
function [ccc_sum]=SEC_C_exact(data,templates,k,moveouts,weights,chan_scale2one)
...
templates=templates-mean(templates);
...
if chan_scale2one==1
    ...
    for j=1:n_s %loop over stations
        for jj=1:n_c %loop over componets
            ...
            % another preprocessing step for exact normalizations
             mv_sum=movsum(s,[m-1,0]);
             sumx2_t=sqrt(movsum(s.^2,[m-1,0])-(mv_sum.^2)./m);
