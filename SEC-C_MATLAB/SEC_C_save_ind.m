%SEC_C_save_ind: Super Efficient Cross-correlation MATlAB code for seismic data 

%SEC-C save individual: This version of SEC-C will save the individual CCC 
%instead of ouputing CCC_sum 
...
function SEC_C_save_ind(data,templates,k,moveouts,weights,CCC_output_dir,chan_scale2one)
...
%Output will be saved in CCC_output_dir 
    ...
    for j=1:n_s %loop over stations
        for jj=1:n_c %loop over componets
            ...
            for i=1:n_t %number of templates
                ...
                %save individual CCCs in CCC_output_dir based on station
                %number,station channles number and template number 
                save([CCC_output_dir,'/CCC_',num2strt(j),'_',num2str(jj),...
                    '_',num2str(i)], ccha(m:l_data));
