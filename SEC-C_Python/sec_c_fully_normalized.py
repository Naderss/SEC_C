...
def SEC_C_exact(data, template, k, moveout, weight, chan_scale2one):
    """ Super Efficient Cross-correlation Python code for seismic data

    SEC_C_exact is designed to handle multiple stations, multiple components
    and multiple templates matched filtering of time series data specifically
    for seismic applications in an efficient time with an efficient memory
    usage. We have adopted the input parameter style as Beauce et al., 2017
    matched filter code (https://github.com/beridel/fast_matched_filter). For
    testing SEC_C_exact user also can use the Beauce et al., 2017 test code.
    ...
    weight : numpy array, shape (n_station, n_template)
        input weights
    chan_scale2one : bool
        true if you want to CCC_sum to be scaled to 1 (divide weights by the
        number of channels), false if you don't want to.
    ...
    # if you want to scale CCC_sum to one set chan_scale2one as True.
    if chan_scale2one:
        weight /= n_component
    ...
    for i in range(n_station):
        ...
        for j in range(n_component):
            ...
            mv_sum = np.cumsum(data_segment, axis=0)
            mv_sum[len_template:, :] = (
                mv_sum[len_template:, :] -
                mv_sum[:-(len_template), :])
            mv_2sum = np.cumsum(data_segment ** 2, axis=0)
            mv_2sum[len_template:, :] = (
                mv_2sum[len_template:, :] -
                mv_2sum[:-(len_template), :])

            data_rms = np.sqrt(mv_2sum - mv_sum ** 2 / len_template)
